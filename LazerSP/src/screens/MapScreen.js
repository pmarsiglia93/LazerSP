...
export default function MapScreen({ navigation }) {
  ...
  const initialRegion = useMemo(() => {
    ...
    return { latitude: -23.5505, longitude: -46.6333, latitudeDelta: 0.18, longitudeDelta: 0.18 };
  }, [location]);
          ...
          {places.filter((p) => p.latitude != null && p.longitude != null).map((place) => {
            ...
            return (
              <Marker
                ...
                onCalloutPress={() => navigation.navigate("Details", { placeId: place.id, place })}
              />
            );
          })}
        </MapView>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  map: { flex: 1 },
  loadingContainer: { flex: 1, alignItems: "center", justifyContent: "center", gap: 16, backgroundColor: theme.colors.background },
  loadingText: { color: theme.colors.textLight, fontSize: 15 },
  overlay: { position: "absolute", top: 20, left: 16, right: 16, backgroundColor: "rgba(255,255,255,0.97)", borderRadius: theme.radius.lg, padding: theme.spacing.md, borderWidth: 1, borderColor: theme.colors.border, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 4 },
  overlayHeader: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 4 },
  title: { fontSize: 16, fontWeight: "800", color: theme.colors.text },
  subtitle: { fontSize: 13, color: theme.colors.textLight, marginBottom: 12, lineHeight: 20 },
  errorRow: { flexDirection: "row", alignItems: "center", gap: 6, marginBottom: 10 },
  errorText: { color: theme.colors.danger, fontSize: 13, fontWeight: "600", flex: 1 },
});
